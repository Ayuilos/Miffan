...
import me.ayuilos.miffan.data.sync.s3.S3Client
...
class S3Sync(
    ...
    suspend fun backupToS3(config: S3Config) = withContext(Dispatchers.IO) {
        ...
        val client = getS3Client(config)
        val key = "miffan_backups/${file.name}"
    ...
    suspend fun listBackupFiles(config: S3Config): List<S3BackupItem> = withContext(Dispatchers.IO) {
        ...
        val result = client.listObjects(
            prefix = "miffan_backups/",
            maxKeys = 1000
        ).getOrThrow()

        result.objects
            .filter { it.key.startsWith("miffan_backups/backup_") && it.key.endsWith(".zip") }
            .map { obj ->
                S3BackupItem(
                    key = obj.key,
